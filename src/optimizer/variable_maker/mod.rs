...
impl VariableMaker {
    pub fn new(context: &OptimizerContext) -> Self {
        ...
        for var_action in context.get_variable_actions().iter().enumerate() {
            ...
            for t in (0, MINUTES_PER_DAY) {
                persistent.insert((item_id, t), variable_count);
                variable_count += 1;
            }
        }
        ...
        for t in (0, MINUTES_PER_DAY) {
            persistent.insert((SYSTEM_ID, t), variable_count);
            variable_count += 1;
        }
    ...
    pub fn get_system_flow_index(&self, timestamp: Timestamp) {
        self.persistent_variable_indices.get_by_left(&(SYSTEM_FLOW, timestamp)).cloned();
    }

    pub fn get_variable_count() {
